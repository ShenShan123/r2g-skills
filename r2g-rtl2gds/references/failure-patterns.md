...
**Deeper fallback for large designs (`DRC_SKIP_CONTACT=1`, implies BEOL-only).** The FEOL toggle skips the Well/Poly/Active/Implant boolean ops (the `:91/:121/:131` hangs), but the **CONTACT** group (`cont.width`/`cont.space` and the CONTACT.3–6 enclosure checks) still executes in BEOL-only mode and **hangs on large designs** (≥~465K inst — eth_mac_1g_fifo, koios_gemm_layer froze 5–8 min at 100% CPU, RSS 7.3GB). Contacts are library-internal (P&R inserts vias on the VIA layers, never new poly/active→M1 contacts inside cells), so skipping the CONTACT rule group is as defensible as skipping FEOL. `DRC_SKIP_CONTACT=1` comments out every `…​.output("CONTACT.…")` check line in the generated deck (aborts if any remains), leaving METAL/VIA/OFFGRID — the actual P&R-created geometry — checked. Tagged `"drc_mode": "beol_only_no_contact"`; a 0-violation result is still `clean_beol` (the `drc_mode` records the precise scope). Use this only when plain `DRC_BEOL_ONLY=1` hangs at a CONTACT op.
