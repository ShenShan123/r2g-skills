...
oracle.  Every writer that can create learner-derived state must therefore
reuse this predicate; keeping it in one module prevents the generic event
writer from becoming a weaker bypass around the online manager.
"""
...
def require_verified_execution(facts) -> None:
    ...
    ``PASS`` and ``FAIL`` are both definitive execution verdicts.  A compile
    or lint result, an unknown oracle, or a partial expanded oracle is not a
    learner-grade verified transition.  Callers intentionally receive the
    same fail-closed error whether they are observing a transition directly
    or appending a derived learner event.
    """
