...
| Tool **detect** | `signoff-loop|def-graph/scripts/flow/_env.sh` (byte-identical, md5 `a5ac873e…`) | Sourced; resolves ORFS + tools + PDK: env → `$R2G_ENV_FILE` → `references/env.local.sh` → `$ORFS_ROOT/env.sh` → `/opt/openroad_tools_env.sh` → `command -v` → well-known paths | ✅ detect-only |
...
**Verification:** eda-install suite **23 passed** (10 bootstrap + 13 tier); signoff-loop **790 / 1**,
...
**Follow-up landed (same branch): conda-staged PDK autodetect.** `_env.sh` (all three copies) now finds
a `open_pdks.sky130a`-staged sky130A under `<conda>/envs/<env>/share/pdk` — closing the gap where a
conda-installed PDK stayed invisible until hand-pinned. It adopts a conda candidate ONLY when it
...
- `_env.sh` — add `R2G_GRAPH_PYTHON` / conda-`eda` discovery so a bootstrapped env is auto-found next
  session. **Edit BOTH copies; a test must assert the md5 stays identical** (CLAUDE.md `a5ac873e…` rule).
...
| `_env.sh` byte-identical invariant (CLAUDE.md `a5ac873e…`) | discovery edits land in both copies; a test asserts md5 equality |
