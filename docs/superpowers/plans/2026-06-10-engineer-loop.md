...
**Architecture:** Two linked SQLite DBs — new gitignored `knowledge/journal.sqlite` (evidence: actions/log_summaries/tool_bugs) and existing git-tracked `knowledge/knowledge.sqlite` + `heuristics.json` (conclusions) — joined by `symptom_id`/`run_id`/`fix_session_id`. A deterministic orchestrator (`scripts/loop/engineer_loop.py`) drives flow→ingest→learn→A/B→promote; unknowns go to an `escalations` queue for the agent tier.
...
- **Already applied in-repo (2026-06-10, before implementation):** the knowledge DB file was renamed `knowledge/runs.sqlite` → `knowledge/knowledge.sqlite` (spec rev 3); all live code/tests/docs use the new name, the `runs` TABLE name is unchanged, and the tasks below already reference the new name.
...
(where `<check>` is drc/lvs/rcx and `$KNOWLEDGE_DIR` is computed the same way as in fix_signoff.sh — add the two-line computation if the script lacks it). Immediately after each script's extract step writes `reports/<check>.json`, add the matching report digest (spec rev 3 — reports generated by EDA tools are summarized too):

```bash
python3 "$KNOWLEDGE_DIR/journal_action.py" report --project "$PROJECT_DIR" \
  --kind <check> --file "$PROJECT_DIR/reports/<check>.json" \
  ${R2G_JOURNAL_DB:+--db "$R2G_JOURNAL_DB"} 2>/dev/null || true
```

No test beyond a smoke `bash -n` syntax check: `bash -n scripts/flow/run_drc.sh scripts/flow/run_lvs.sh scripts/flow/run_rcx.sh`.
...
Also add the decision-10 catch-all report sweep (spec rev 3) as a module function, called right BEFORE the back-fill above (so the new rows get their `run_id` filled by it). Idempotent: deletes this project's previous `tool='report'` rows first (mirrors the `failure_events` rebuild pattern):
...
Call order at the end of `ingest()`: `_journal_report_digests(project)` then the run_id back-fill block (back-fill fills the sweep's fresh NULL `run_id`s).
...
Add a "Engineer Loop (spec 2026-06-09)" section: the two-DB table (journal.sqlite = evidence, gitignored; knowledge.sqlite/heuristics.json = conclusions, tracked), the new tables (actions/log_summaries/tool_bugs; recipe_status/ab_trials/escalations/meta), the lifecycle diagram (shadow→candidate→promoted, A/B gated), and invariant additions: (16) only promoted recipes rank live; (17) journal archival loses no conclusions; (18) provenance chain queryable via trace_provenance.py.
...
Pick the smallest existing clean nangate45 project from `design_cases/` (use `tools/sky130_campaign.py`-style query or just `sqlite3 knowledge/knowledge.sqlite "SELECT project_path FROM runs WHERE platform='nangate45' ORDER BY cell_count LIMIT 1"`). Then:
...
```bash
git add r2g-rtl2gds/knowledge/heuristics.json r2g-rtl2gds/knowledge/knowledge.sqlite
